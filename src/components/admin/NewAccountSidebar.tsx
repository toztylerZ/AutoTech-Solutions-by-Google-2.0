...
import { Save, User, Phone, Shield, X, PanelLeftClose, Eraser, Building, LayoutGrid } from 'lucide-react';
import { useAdminStore } from '../../store/adminStore';
import { motion, AnimatePresence } from 'motion/react';

export default function NewAccountSidebar() {
  const { 
    triggerRefresh, 
  ...
  const handleCreate = async () => {
    if (!formData.user_name || !formData.phone || !formData.login || !formData.password) {
    ...
    try {
      ...
      if (res.ok) {
        setFeedback({ type: 'success', msg: 'Аккаунт успешно создан!' });
        triggerRefresh();
        setTimeout(() => {
      ...
      console.error(err);
      setFeedback({ type: 'error', msg: 'Критическая ошибка' });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-full bg-graphite-light rounded-r-3xl overflow-hidden">
      <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest">новый аккаунт</div>
            <div className="text-xs font-bold text-white uppercase tracking-wider">Регистрация сотрудника</div>
          </div>
        </div>
        <button 
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={handleChange}
              disabled={formData.role === 'администратор' || formData.role === 'менеджер'}
      ...
      <div className="p-4 border-t border-white/10 bg-black/40 backdrop-blur-md">
        ...
        >
          {saving ? 'Создание...' : (
            <>
              <Save className="w-4 h-4" /> Создать аккаунт
            </>
          )}
        </button>
      </div>
